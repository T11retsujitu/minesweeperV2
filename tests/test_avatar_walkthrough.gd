...
func _test_avatar_victory_line(t):
	...
	_assert_state(t, state, 2, 2, 8, Vector2i(2, 4), 5, "avatar victory T5")
	t.equal(_event_types(t5), ["player_moved", "countdown_paused"], "avatar victory T5 events")
	...
	_assert_state(t, state, 2, 2, 8, Vector2i(2, 4), 6, "avatar victory T6")
	t.equal(_event_types(t6), ["cells_revealed", "countdown_paused"], "avatar victory T6 events")
	t.equal(_event_cells(t6, "cells_revealed").size(), 11, "avatar victory T6 flood count")
	t.equal(_event(t6, "countdown_paused").get("countdown", -1), 2, "avatar victory T6 paused countdown")
	t.check(not _has_player_damage_source(t6, "enemy_attack"), "avatar victory T6 no enemy attack damage")

	var t7 = _flag_and_detonate(state, Vector2i(0, 3))
	_assert_state(t, state, 0, 2, 8, Vector2i(2, 4), 7, "avatar victory T7")
...
func _test_avatar_defeat_line(t):
	...
	_assert_state(t, state, 2, Balance.ENEMY_COUNTDOWN, 2, Vector2i(3, 2), 4, "avatar defeat T4")
	t.equal(_event_types(t4), ["player_moved", "countdown_paused"], "avatar defeat T4 events")
	...
	_assert_state(t, state, 2, Balance.ENEMY_COUNTDOWN, -1, Vector2i(3, 2), 5, "avatar defeat T5")
	t.equal(_event_types(t5), ["cells_revealed", "mine_exploded", "enemy_damaged", "player_damaged", "countdown_paused", "defeat"], "avatar defeat T5 events")
