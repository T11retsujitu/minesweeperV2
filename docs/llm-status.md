...
- **現フェーズ**: **見た目インパクト強化計画 M-0〜M-E 完了**(2026-07-17、計画: `~/.claude/plans/functional-weaving-frost.md`。ユーザー判断「現状はインパクトが弱い」→ テーマ=ダンジョン探索(PV踏襲)+AI生成アセット+起爆モーメント誇張 = pv-vision-roadmap Step 2 のコード側)。現在は**テーマ近似プレースホルダで稼働中** — 次タスクは (1) **ユーザーが docs/asset-spec.md §4 のプロンプトで画像生成 → assets_src/ に配置 → pipeline 実行で差し替え**(コード変更不要) (2) 実プレイ評価(新ビジュアル+12×12+縄張り半径1+起爆の手応え)。Phase 1 成果物は ruleset="phase1" として保存(テスト230件で検証継続)。
...
- **UX改良 M3a 縄張りルール(§7.4、2026-07-16。同日 半径2→1 改訂=D31改訂参照)**: `TERRITORY_RADIUS=1`(game_balance、敵中心3×3)。avatar のみ、アクション解決後のプレイヤー位置が縄張り外ならカウントダウン凍結(`countdown_paused`)・敵攻撃なし。盤面に縄張りティント常時表示(敵死亡で消灯)+HUD「Countdown: N (paused)」。敵隣接セルは距離2(圏外)から無リスク開放可能。**半径1でのオラクル: 勝利7T最終HP8(被弾1回)/敗北5T(HP値は半径2時と同一)**。設計判断 D31(改訂経緯・トレードオフ込み)
...
- **見た目インパクト強化 M-A 盤面テクスチャ化(2026-07-17)**: cell_node の StyleBoxFlat 塗りを全テクスチャ化(hidden=石ブロック88×98厚み込み・_b微差分を決定的ハッシュで混在 / floor=暗い床 / detonated=クレーター「X」廃止 / flagged=爆弾樽 / debug地雷=樽0.6倍)。数字= Press Start 2P+黒縁取り(色は fx_config.COLOR_NUMBER_LEVELS)。ヒートマップ廃止→床上の低アルファ暖色ティント(COLOR_HEAT_TINT_LEVELS、0-2=alpha 0)。CELL_INSET_PX=0(シームレス)。BoardWorld に TEXTURE_FILTER_NEAREST。プレースホルダはテーマ近似の手続き描画(dungeon24 準拠)に改良済み。**注意: PNG 差し替え後は `godot --headless --path . --import` で再インポートしないと実行時に旧テクスチャが出る**。検証: テスト1558不変・WSLg(固定盤面/樽/12×12最小ズーム数字判読/INSET0後click的中)。既知: 敵トークンがセル数字を覆う(プレースホルダ由来、M-B のスプライト化で対応)
- **見た目インパクト強化 M-B キャラスプライト+背景(2026-07-17)**: player/enemy トークンを `body_root`(Node2D、M-D ノックバックの演出オフセット受け皿)配下の Sprite2D 化(足元= entity_anchor 契約維持、影とバッジは draw 継続)。2フレームアイドルは各トークン内永続 Timer(fx_config.IDLE_FRAME_SEC=0.5、敵は半位相ずらし。**学び: timeout ハンドラ内の wait_time 変更は start() を呼ばないと即適用されない**)。背景= bg_dungeon.png の TextureRect(KEEP_ASPECT_COVERED・IGNORE)。敵セルの数字はスライムの頭上に覗いて可読。検証: テスト1558不変・アイドル切替はヘッドレスプローブで機械検証(スクショは quit-frames **前**に撮られるため静止検証しかできない点に注意)・WSLg で接地/Yソート/移動確認
- **見た目インパクト強化 M-C HUD整理(2026-07-17)**: 通常HUDを「HPバー×2+地雷/フラグカウンタ(icon×N)+status+Retry/Help小ボタン」に縮約(hud_panel StyleBoxTexture+Press Start 2P)。seed/turn/input/ログ/モード切替/Show Mines は下部折りたたみ Debug ドロワーへ(`OS.is_debug_build()` false で非表示、ノード参照は常時生成で battle_feedback/デバッグコマンド互換維持)。盤面スロット拡大。検証: テスト1558不変・WSLg(HUD4要素・click的中・全debugコマンドスモーク・起爆+retry連打ハングなし)
- **見た目インパクト強化 M-D 起爆モーメント誇張(2026-07-17)**: 意図起爆を多段演出化 — 白フラッシュ+橙ビネット(0.12s)→ hit_stop 並行 **punch-zoom**(camera_rig.punch_scale 1.0→1.12→1.0、`view_center/zoom_scalar` 非破壊の直交合成: `effective_center = T + (C-T)/s`。reset_punch は fx_layer.clear_all から)→ 火球3F(加算)+パーティクル32 → 衝撃波リング+隣接フラッシュ+敵ノックバック(body_root)+白フラッシュ → 煙。総ブロック 0.80s(意図起爆のみ、dud/誤爆は従来)。全定数 fx_config(PUNCH_ZOOM_* 等)。検証: テスト1558不変・WSLg confirmburst 連番(閃光→火球+ズーム→リング→クレーター)・**起爆前後 zoomstate 完全一致**・演出中 retry 連打で残留/ハングなし
