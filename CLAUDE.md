...
- テスト: `~/.local/bin/godot --headless --path . --script res://tests/run_tests.gd` — 1558件・exit 0 が正常。domain/generation 変更後は必須
- fixture `phase1_core_demo` の座標は変更禁止(検証済みオラクル)。数値調整は `scripts/config/game_balance.gd` のみ
- git: push は**ユーザーの明示指示があったときのみ**(自発的 push 禁止)。マイルストーン毎にローカルコミット
